...
Since the true hidden representations of the data samples are not available in the dataset, they must be found by computing the expected value belonging to a certain sample:

$$z^i = \mathbb{E}_{P(Z|X=x^i)}[Z|X]$$

where

$$P(Z | X = x^{(i)}) = \frac{p_{\text{model}}(x, z)}{p_{\text{model}}(x)} = \frac{p_{\text{model}}(x, z)}{\int_{-\infty}^{\infty} p_{\text{model}}(x, z) \,dz}$$

Since the formula for conditional distribution contains intractable integral, it cannot be used to perform the **Expectation Maximization** algorithm, which consists in iterative obtaining the most suitable variables $$z$$ and using them to perform an MLE step. To overcome this, the **Variational Inference** technique may be used, which consists in approximating the untractable distribution $$P(Z|X)$$ with a new parametrized distribution $$Q(Z; \phi)$$, where the parameters are to be derived from the $$X$$. The optimal distribution is to be found by minimizing the **Kullback-Leibler Divergence**:

$$Q^*(Z) = \arg\min_{\phi}\mathbb{KL}[Q(Z;\phi)||P(Z|X)]$$

where

$$\mathbb{KL}[p_1(x)||p_2(x)]:=\int_{-\infty}^{\infty} p_1(x)log\frac{p_1(x)}{p_2(x)}\,dx$$

The optimization problem can be simplified by unfolding the KL-divergence to find the formulation of the **Evidence Lower Bound** (ELBO):

$$\text{KL}[Q(Z; \phi) \,||\, P(Z | X)] 
= \mathbb{E}_Q[\log Q(Z; \phi)] - \mathbb{E}_Q[\log P(Z | X)] \\
= \mathbb{E}_Q[\log Q(Z; \phi)] - \mathbb{E}_Q\left[\log \frac{P(X, Z)}{P(X)}\right] \\
= \mathbb{E}_Q[\log Q(Z; \phi)] - \mathbb{E}_Q[\log P(X, Z)] + \log P(X) \\ 
= logP(X) - ELBO(Q)$$

Finally, the ELBO can be broken down into two tractable parts:

$$ELBO(Q) = \mathbb{E}_Q[\log P(X | Z)] + \mathbb{E}_Q[\log P(Z)] - \mathbb{E}_Q[\log Q(Z; \phi) \\

= \mathbb{E}_Q[\log P(X | Z)] - \text{KL}[Q(Z; \phi) \,||\, P(Z)]$$

The ELBO can be maximized by simultaneously maximizing the conditional log-likelihood of the data and minimizing the KL-divergence between the approximated distribution and the prior distribution of the latent space, which in practice can be chosen randomly.

From the formulation of the ELBO arise two significant conclusions:
1. By maximizing it, the original problem of the Variational Inference is optimized.
2. Since KL-divergence is always non-negative, ELBO is a true lower bound approximation of the untractable evidence $$P(X)$$. 

#### Denoising Diffusion Probabilistic Models

On high level, the concept of Denoising Diffusion Probabilistic Models (DDPM) can be described as follows: 1) A sample from the data distribution is passed through a sequence of `T` steps, where on each step a small portion of Gaussian noise is added to it. 2) Ass soon as the sample is undistinguishable from pure noise, the denoising process begins, which consists in removing the respective amounts of noise at each step, until the sample is the same as before noising. 3) A probabilistic model, having learned the conditional distributions between consecutive steps for multiple data samples, denoises newly sampled pure noise into a new data sample from the data distribution.

As stated in [generative-models-ddpm-denoising](#generative-models-ddpm-denoising), the first step in formulation of the DDPM concept is augmentation of the data from distribution $$p(x)$$ with zero mean and unit variance into `T` groups, where each subsequent group is characterized by higher noise level. During the noising process data at each steps is sampled from a respective conditional distribution:

$$x^{(0)} \sim q(x^{(0)}), x^{(1)} \sim q(x^{(1)}|x^{(0)}), ..., x^{(T)} \sim q(x^{(T)}|x^{(T-1)})$$

where $$q(x^{(0)})$$ is the data distribution. The $$q(x^{(T)})$$, which is the data with highest amount of noise, is intended to be pure Gaussian noise. For this reason all distributions $$q(x^{(t)}|x^{(t-1)})$$ are Gaussian distributions parametrized by chosen $$\beta_{1:T}$$ values. Then they can be formulated as:

$$q(x^{(t)}|x^{(t-1)}) := \mathcal{N}(\sqrt{1-\beta_t}x^{(t-1)},\beta_t) = \sqrt{1-\beta_t}x^{(t-1)} + \sqrt{\beta_t}\epsilon$$

where $$\epsilon$$ is sampled from $$\mathcal{N}(0, 1)$$. In order to make the conditional distributions of the denoising process simple, parametrized Gaussian distributions are used:

$$p(x^{(t-1)} | x^{(t)}, \theta) = \mathcal{N} \left( x^{(t-1)}; \mu_{\theta}(x^{(t)}, t-1), \sigma^2_{\theta}(x^{(t)}, t-1) \right)
$$

Here All distributions are parametrized by shared parameters $$\theta$$, what indicates that they are going to use a shared neural network for simpler regression tasks. The reason to use shared parameters for all distributions is that all neighboring conditional distributions are expected to be similar and therefore don't need separate parameters. Each of the Gaussian distributions use mean and variance derived separately at each step from the previous data.

The forward diffusion pass is assumed to be a **first-order Gaussian auto-regressive process**, i.e. each subsequent distribution is a Gaussian, depending on only one previously predicted step, the formula for the total log-likelihood can be easily broken down into individual parts (see the formula for likelihood in autoregressive models in [autoregressive-models](#autoregressive-models)):

$$\mathcal{L}(\theta) = \mathbb{E}_{q(x_{0:T})} \left[ \log \left( p(x^{(T)}) \prod_{t=1}^T p(x^{(t-1)} \mid x^{(t)}; \theta) \right) \right] \\
= \mathbb{E}_{q(x^{(T)})} [\log p(x^{(T)})] + \sum_{t=1}^T \mathbb{E}_{q(x^{(t-1)}, x^{(t)})} [\log p(x^{(t-1)} \mid x^{(t)}; \theta)]$$

Since only the second part of the expression depends on the parameters, the maximization of the log-likelihood is reduced to optimizing individual parts of the sum of expectations over distributions at respective steps:

$$\mathcal{L}_{t-1}(\theta) = \mathbb{E}_{q(x^{(t-1)},x^{(t)})} \left[ \log p(x^{(t-1)} \mid x^{(t)}; \theta) \right] \\
= \mathbb{E}_{q(x^{(0)},x^{(t-1)},x^{(t)})} \left[ \log p(x^{(t-1)} \mid x^{(t)}; \theta) \right] \\
= \mathbb{E}_{q(x^{(0)}, x^{(t)})} \left[ \mathbb{E}_{q(x^{(t-1)}|x^{(0)},x^{(t)})} \left[ \log p(x^{(t-1)} \mid x^{(t)}; \theta) \right] \right]$$

In order to compute the inner expectation, using the formula for Gaussian distribution, two additional distributions must be derived: the $$q(x^{(t)}|x^{(0)})$$ and $$q(x^{(t-1)}|x^{(0)},x^{(t)})$$ (the full derivations can be found in [autoregressive-models](#autoregressive-models)).

$$q(x^{(t)} | x^{(0)}) = \mathcal{N} \left(\left( \prod_{t'=1}^t \sqrt{1 - \beta_{t'}} \right) x^{(0)}, 1 - \prod_{t'=1}^t (1 - \beta_{t'}) \right).
$$

$$q(x^{(t-1)}|x^{(0)},x^{(t)}) = \mathcal{N}(\mu_{t-1|0,t}, \sigma^2_{t-1|0,t})
$$

where $$\mu_{t-1|0,t}, \sigma^2_{t-1|0,t}$$ are obtained from $$x_0$$, $$x_t$$ and $$\beta$$ parameters.

This leads to the final form of the partial log-likelihood to be optimized:

$$\mathcal{L}_{t-1}(\theta) = -\frac{1}{2}\mathbb{E}_{q(x^{(0)},x^{(t)})} \left[ \frac{(\mu_{t-1|0,t}-\mu_{\theta}(x^{(t)},t-1)^2+\sigma^2_{t-1|0,t})}{\sigma^2_{\theta}(x^{(t)},t-1)} + \log2\pi\sigma^2_{\theta}(x^{(t)},t-1) \right]$$

Since optimizing the parametrized variance has been observed to be a rather difficult task, it is often replaced with the variance from the noising process. In order to maximize the log-likelihood, the parametrized mean is optimized to be as close to the $$\mu_{t-1|0,t}$$ as possible.

The last step is redefinition of the optimization task, so that it involves the trainable neural network performing regression. One of the approaches is to train a model predicting the $$x^{(0)}$$ from the $$(x^{(t)},t-1)$$ and than compute the $$\mu_{t-1|0,t}$$ and use it to sample the data at the previous denoising step $$x^{(t-1)}$$.
